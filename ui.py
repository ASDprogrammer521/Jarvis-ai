...
class CompactMicWindow(QWidget):
    """Tiny always-on-top mic button shown when the main window is in compact mode.
    ...
    def paintEvent(self, _) -> None:
        ...
        g.setColorAt(0.7, QColor(col.red(), col.green(), col.blue(), 18))
        g.setColorAt(1.0, QColor(0, 0, 0, 0))
        p.setPen(Qt.PenStyle.NoPen)
        p.setBrush(QBrush(g))
        p.drawEllipse(QRectF(0, 0, 56, 56))

        # Main disc
        p.setBrush(QColor(1, 13, 20, 230))
        p.setPen(QPen(col, 2.0))
        p.drawEllipse(QRectF(6, 6, 44, 44))

        # Pulsing outer arc when active
        if not self._muted and (self._listening or self._amp > 0.05):
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.setPen(QPen(col, 2.2))
            span = int(40 + 80 * self._amp)
            base = int((self._phase * 57) % 360)
            p.drawArc(QRectF(3, 3, 50, 50), base * 16, span * 16)

        # Mic icon (simple geometric)
        p.setPen(Qt.PenStyle.NoPen)
        p.setBrush(col)
        # capsule
        p.drawRoundedRect(QRectF(cx - 5, cy - 11, 10, 16), 5, 5)
        # stand
        p.setPen(QPen(col, 2.0))
        p.drawLine(QPointF(cx, cy + 5), QPointF(cx, cy + 11))
        p.drawLine(QPointF(cx - 6, cy + 11), QPointF(cx + 6, cy + 11))
        # arc around capsule
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawArc(QRectF(cx - 9, cy - 9, 18, 18), 200 * 16, 140 * 16)
    ...
    def mouseReleaseEvent(self, e) -> None:
        if e.button() == Qt.MouseButton.LeftButton:
            if not getattr(self, "_moved", False):
                # short click → toggle
                if time.time() - getattr(self, "_press_time", 0) < 0.4:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._right_panel.setVisible(False)  # shown only when Chat tab is active
        ...
        _cmd_lay.setSpacing(0)
        _cmd_lay.addLayout(self._build_input_row())
        root.addWidget(self._cmd_bar_host)

        root.addWidget(self._build_footer())

        # Quick-access drawer (kept for compatibility; Settings uses full-screen page)
        self._quick_drawer = self._build_quick_drawer()
        self._quick_drawer.hide()

        # Full-screen settings page (shown when Settings tab is active)
    ...
    def closeEvent(self, event):
        # Clean up floating compact mic so it never outlives the app.
        try:
            if getattr(self, "_compact_sync_tmr", None) is not None:
                self._compact_sync_tmr.stop()
            if getattr(self, "_compact_mic", None) is not None:
                self._compact_mic.close()
                self._compact_mic = None
        except Exception:
            pass
    ...
    def _build_header(self) -> QWidget:
        ...
        tabs.setStyleSheet(
            "QWidget#NavTabs { background: transparent; }"
        )
        tlay = QHBoxLayout(tabs)
        tlay.setContentsMargins(4, 4, 4, 4)
        tlay.setSpacing(6)

        def _make_tab(label: str, key: str) -> QPushButton:
            btn = QPushButton(label)
            ...
            btn.setStyleSheet(f"""
                ...
                QPushButton:checked {{
                    background: qlineargradient(x1:0,y1:0,x2:1,y2:0,
                        stop:0 rgba(255,180,40,0.35), stop:1 rgba(255,140,20,0.45));
                    color: #ffe9a8;
                    border: 1px solid rgba(255,200,60,0.65);
                }}
            """)
            btn.clicked.connect(lambda _=False, k=key: self._on_nav_tab(k))
            return btn

        self._tab_dashboard = _make_tab("Dashboard", "dashboard")
        self._tab_chat = _make_tab("Chat", "chat")
        self._tab_settings = _make_tab("Settings", "settings")
        tlay.addWidget(self._tab_dashboard)
        tlay.addWidget(self._tab_chat)
        tlay.addWidget(self._tab_settings)
        lay.addWidget(tabs)
        lay.addStretch()

        # Keep hidden stubs so old code that references them does not crash
    ...
    def _on_nav_tab(self, key: str) -> None:
        ...
        if key == "dashboard":
            if hasattr(self, "_hud_cam_stack"):
                self._hud_cam_stack.setCurrentIndex(0)
            prev = getattr(self, "_prev_nav", None)
            if prev and prev != "dashboard" and hasattr(self, "_open_remote"):
                try:
                    self._open_remote()
                except Exception:
                    pass
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        hdr.setStyleSheet(
            ...
            f"border-bottom: 1px solid {C.BORDER_A}; padding-bottom: 8px; letter-spacing: 2px;"
        )
        lay.addWidget(hdr)
        lay.addSpacing(4)
        ...
        info_panel.setStyleSheet(
            f"background: {C.PANEL2}; border: 1px solid {C.BORDER_A}; border-radius: 6px;"
        )
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        pulse = QLabel("●")
        pulse.setFont(QFont("Segoe UI", 10))
        ...
        chat_hdr.addWidget(title)
        chat_hdr.addStretch()
        lay.addLayout(chat_hdr)
        ...
        # Hidden drop zone so + button / file upload still works
        ...
        # Hidden mute/interrupt kept for API compatibility
        self._interrupt_btn = QPushButton()
        self._interrupt_btn.setVisible(False)
    ...
    def _build_settings_page(self) -> QWidget:
        """Full-screen settings — hosts the same controls as the old ⚙ drawer."""
        ...
        close_btn.setStyleSheet(f"""
            QPushButton {{
                ...
                border: 1px solid {C.BORDER_A}; border-radius: 12px; padding: 0 14px;
            }}
        ...
        scroll.setStyleSheet(f"""
            ...
            QScrollBar:vertical {{
                background: {C.PANEL}; width: 10px; border-radius: 5px;
            }}
            QScrollBar::handle:vertical {{
                background: {C.BORDER_B}; border-radius: 5px; min-height: 40px;
            }}
        """)
        ...
        # Re-parent the original settings drawer so ALL old buttons remain
        if hasattr(self, "_quick_drawer") and self._quick_drawer is not None:
            d = self._quick_drawer
            d.setParent(body)
            d.show()
            d.setVisible(True)
            # Remove fixed floating-panel constraints
            ...
            # Drop any leftover floating geometry
            d.adjustSize()
            d.setStyleSheet(f"""
                QWidget#QuickDrawer {{
            ...
            """)
            # Ensure every child button is visible and expands horizontally
            for child in d.findChildren(QPushButton):
                child.setVisible(True)
                child.setMinimumHeight(30)
                child.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
            bl.addWidget(d, stretch=0)
            bl.addStretch(0)
        else:
            miss = QLabel("Settings controls failed to load.")
            miss.setStyleSheet(f"color: {C.RED};")
            bl.addWidget(miss)
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        compact_btn.setToolTip(
            "Hide the full window and leave only a floating mic icon. "
            "Click the icon to mute/unmute, double-click to restore.")
        compact_btn.clicked.connect(self._enter_compact_mode)
    ...
    def _position_quick_drawer(self):
        if not hasattr(self, '_quick_drawer'):
            return
        # When settings page owns the drawer, do not apply floating geometry
        if getattr(self, "_active_nav", "") == "settings":
            return
        if hasattr(self, "_settings_page") and self._settings_page.isVisible():
            return
        _W = 220
    ...
    def _build_input_row(self) -> QHBoxLayout:
        """Modern 'Ask anything' bar — matches clean chat UIs."""
        row = QHBoxLayout()
        row.setSpacing(8)
        ...
        plus.setFont(QFont("Segoe UI", 16))
        plus.setStyleSheet(f"""
            QPushButton {{
                background: transparent; color: {C.TEXT_DIM};
                border: none; border-radius: 16px;
            }}
            QPushButton:hover {{ color: {C.WHITE}; background: rgba(255,255,255,0.06); }}
        """)
        plus.setToolTip("Attach file")
        plus.clicked.connect(lambda: self._drop_zone._browse() if hasattr(self, "_drop_zone") else None)
        bl.addWidget(plus)
        ...
        mic.setToolTip("Toggle microphone")
        mic.setStyleSheet(f"""
            QPushButton {{
                background: transparent; color: {C.TEXT_MED};
                border: none; border-radius: 17px; font-size: 14px;
            }}
    ...
    def _refresh_hud_btn(self):
        ...
        # Cycle of three centrepieces — none is "off".
        style = f"""
    ...
    def _enter_compact_mode(self) -> None:
        ...
        if getattr(self, "_compact_mic", None) is None:
            self._compact_mic = CompactMicWindow()
            self._compact_mic.restore_requested.connect(self._exit_compact_mode)
            self._compact_mic.toggle_requested.connect(self._toggle_mute)
        # Sync current state
        try:
            amp = float(getattr(self.hud, "_amp_disp", 0.0) or 0.0)
        except Exception:
            amp = 0.0
        self._compact_mic.set_state(
            muted=bool(getattr(self, "_muted", False)),
            listening=str(getattr(self.hud, "state", "")).upper() == "LISTENING",
            amp=amp,
        )
        self._compact_mic.show()
        self._compact_mic.raise_()
        self.hide()
        # Keep the mic icon in sync with live state
        if not hasattr(self, "_compact_sync_tmr") or self._compact_sync_tmr is None:
            self._compact_sync_tmr = QTimer(self)
            self._compact_sync_tmr.timeout.connect(self._sync_compact_mic)
            self._compact_sync_tmr.start(200)
        else:
            self._compact_sync_tmr.start(200)
        self._log.append_log("SYS: Compact mode — floating mic active. Double-click icon to restore.")

    def _exit_compact_mode(self) -> None:
        """Restore the full window from the floating mic."""
        if getattr(self, "_compact_sync_tmr", None) is not None:
            self._compact_sync_tmr.stop()
        if getattr(self, "_compact_mic", None) is not None:
            self._compact_mic.hide()
        self.show()
        self.raise_()
        self.activateWindow()
        self._log.append_log("SYS: Full window restored.")

    def _sync_compact_mic(self) -> None:
        mic = getattr(self, "_compact_mic", None)
        if mic is None or not mic.isVisible():
            return
        try:
            amp = float(getattr(self.hud, "_amp_disp", 0.0) or 0.0)
        except Exception:
    ...
    def _style_mute_btn(self):
        if self._muted:
            ...
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: rgba(255, 77, 109, 0.12); color: {C.MUTED_C};
                    border: 1px solid {C.MUTED_C}; border-radius: 12px;
                }}
                QPushButton:hover {{ background: rgba(255, 77, 109, 0.22); }}
            """)
        else:
            self._mute_btn.setText("🎙  MICROPHONE ACTIVE")
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: rgba(55, 255, 139, 0.10); color: {C.GREEN};
                    border: 1px solid {C.GREEN}; border-radius: 12px;
                }}
                QPushButton:hover {{ background: rgba(55, 255, 139, 0.20); }}
            """)
